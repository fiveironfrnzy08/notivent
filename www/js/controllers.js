angular.module('notivent.controllers', [])
	...
	.controller('FeedController', ['$rootScope', '$scope', '$http', 'OAuth', '$ionicPlatform', 'WebSQL', '$q', '$timeout', '$cordovaPush', '$ionicUser',
		function ($rootScope, $scope, $http, OAuth, $ionicPlatform, WebSQL, $q, $timeout, $cordovaPush, $ionicUser) {
			...
			hmac.start('sha1', 'q8ZPE4pFF0qpt64700KQ++VS+bXRw5UGjZi8Bhrz');
			hmac.update('Test message');
			var signature = hmac.digest().toHex()
			$http.post('https://a.notivent.me', {
				headers: {'Authorization': 'AWS AKIAJA55OL47TQV43OKA:' + signature}
			}).then(function(a, b){
				console.log(a);
				console.log(b);
			});
			...
			//};
			//
			//$cordovaPush.register().then(function(result){
